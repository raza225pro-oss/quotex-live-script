...
function hideBonusBanner() {
  ...
  if (found) {
    ...
    while (
      ...
      if (target.parentElement.offsetHeight > 200) break;
    ...
    <div style="background:#1c1c2e;padding:26px;border-radius:16px;width:330px;
      ...
      <div style="display:flex;gap:8px;margin-bottom:10px;">
        <button id="_btn_profit"
          style="flex:1;padding:10px;border:2px solid ${!isLoss ? '#0faf59' : '#444'};
                 background:${!isLoss ? '#0faf59' : 'transparent'};
                 color:#fff;border-radius:8px;cursor:pointer;font-size:13px;font-weight:bold;">
          Profit
        </button>
        <button id="_btn_loss"
          style="flex:1;padding:10px;border:2px solid ${isLoss ? '#ff3e3e' : '#444'};
...
if (savedKey) {
  checkLicense(savedKey).then(valid => {
    if (valid) { init(); }
