...
function _runUpdateUI() {
  ...
  // ── Demo balance → Leaderboard ──
  // b.YnoT0 se demo balance padho aur leaderboard elements mein dikhao
  let curBal = 0;
  ...
  if (curBal <= 0) {
    ...
    curBal = balEl ? safeNum(balEl.textContent) : 0;
  }
  document.querySelectorAll('.Zt1hG').forEach(el => {
    el.textContent = fmtAmt(curBal);
  });
